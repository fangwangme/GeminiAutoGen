...
所有时间参数都可以在 **设置** 中修改（单位：秒）。

| 设置项                 | 默认值          | 说明                                  |
| ---------------------- | --------------- | ------------------------------------- |
| 图片生成超时           | 120 秒（2 分钟）| Gemini 生成最大等待时间              |
| 下载等待超时           | 120 秒（2 分钟）| 等待文件出现在源文件夹                |
| 页面加载/稳定          | 30 秒           | 等待页面加载并稳定图片                |
| 输入框超时             | 5 秒            | 等待提示输入出现                      |
...
如果页面加载很慢，可以在设置里调高**操作间隔（Action Step Delay）**。它也会同时影响这些由它计算出的等待时间：
- 标签页加载完成后的额外等待：`stepDelay * 2`
- 发送按钮等待超时：`max(输入框超时, stepDelay * 5)`
- 页面稳定宽限时间：`min(页面加载/稳定超时, max(轮询间隔, stepDelay) * 2)`
- UI 操作间的短暂停顿：`stepDelay`、`stepDelay * 2`、`stepDelay * 3`、`stepDelay / 2`、`max(200ms, stepDelay / 5)`
